...
- **`sci push` tests base + overlay, NOT the pushed commit.** `ci-rsync.sh` creates a worktree at `origin/HEAD` (the base) and then rsyncs the developer's **entire local working tree** on top (`rsync -a $CI_RSYNC_ARGS --filter=':- .gitignore' --exclude=.git .` — no `--exclude='*'`, so all non-gitignored files including tracked `src/` overlay the base). So **local uncommitted/unpushed changes ARE tested** — you do NOT need to push to GitHub for a local fix to be picked up. The `Preparing worktree (detached HEAD <sha>)` line is just the base, not what gets tested. `CI_RSYNC_ARGS --include` rules only force gitignored corpora into the overlay; they don't restrict it. To test exactly a committed commit with no overlay, use the HTTP path (`POST /job` with repo+commit+script). See the `ci-rsync.sh` header comment for the full explanation.
